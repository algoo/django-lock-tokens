...
class SessionLockingTestCase(TransactionTestCase):
    ...
    def test_view_decorators(self):
        ...
        r = c.get(
            reverse("view-that-locks-object-1"),
        ...
        r = c.get(
            reverse("view-that-locks-object-2", args=[self.test_model_instance.id])
        )
        self.assertEqual(r.status_code, 200)
        self.assertTrue(self.test_model_instance.is_locked())
        ...
        r = c.get(
            reverse("view-that-unlocks-object", args=[self.test_model_instance.id])
        )
        self.assertEqual(r.status_code, 200)
        self.assertFalse(self.test_model_instance.is_locked())
