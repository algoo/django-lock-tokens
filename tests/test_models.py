...
class LockableModelTestCase(TransactionTestCase):
    ...
    def setUp(self):
        self.test_model_instance = TestModel.objects.create(name="test LockableModel")
    ...
    def test_locking_scenario(self):
        ...
        self.assertIsInstance(
            token_dict, dict, "The lock method should return a " "dictionary instance"
        )
        self.assertIn(
            "token",
            token_dict.keys(),
            "The token dictionary should " "contain a 'token' key",
        )
        self.assertIsInstance(token_dict["token"], str)
        self.assertIn(
            "expires",
            token_dict.keys(),
            "The token dictionary should " "contain a 'expires' key",
        )
        self.assertIsInstance(token_dict["expires"], str)

        # Check that object is locked
        self.assertTrue(
            self.test_model_instance.is_locked(),
            "The is_locked method" " should return True since the object is locked",
        )

        # Try to access it without providing token
        with self.assertRaises(
            AlreadyLockedError,
            msg="Trying to lock already "
            "locked object without valid token should raise an AlreadyLockedError",
        ):
            self.test_model_instance.lock()
        with self.assertRaises(
            UnlockForbiddenError,
            msg="Trying to unlock already "
        ...
        time.sleep(1)
        new_token_dict = self.test_model_instance.lock(token=token_dict["token"])
        self.assertEqual(
            token_dict["token"],
            new_token_dict["token"],
            "The token " "string should not have changed when renewing token",
        )
        self.assertNotEqual(
            token_dict["expires"],
            new_token_dict["expires"],
            "The "
        ...
        self.assertFalse(
            self.test_model_instance.is_locked(),
            "The is_locked " "method should return False since the object is unlocked",
        )
    ...
    def test_locking_scenario_using_proxy(self):
        ...
        # Check that object is locked
        self.assertTrue(
            LockableModel.is_locked(self.regular_model_instance),
            "The is_locked method" " should return True since the object is locked",
        )
        ...
        # Renew lock token for object
        time.sleep(1)
        new_token_dict = LockableModel.lock(
            self.regular_model_instance, token=token_dict["token"]
        )
        self.assertEqual(
            token_dict["token"],
            new_token_dict["token"],
    ...
    def test_expired_lock(self):
        ...
        lock_token.locked_at = lock_token.locked_at - datetime.timedelta(
            seconds=TIMEOUT + 1
        )
        lock_token.save()
        ...
        self.assertNotEqual(
            ...
            new_lock_token.token_str,
            "The " "new token string should not be equal to the expired token one",
        )
