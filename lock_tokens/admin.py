...
from lock_tokens.sessions import get_session_key
from lock_tokens.sessions import lock_for_session
from lock_tokens.sessions import unlock_for_session
...
class LockableModelAdmin(admin.ModelAdmin):
    def render_change_form(
        ...
        return super(LockableModelAdmin, self).render_change_form(
    ...
    def change_view(self, request, object_id, form_url="", extra_context=None):
        ...
        return super(LockableModelAdmin, self).change_view(
...
class LockedContentTypesFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return (
            (c.id, c.name)
            for c in ContentType.objects.annotate(nb_locks=Count("locktoken")).filter(
                nb_locks__gt=0
            )
        )
