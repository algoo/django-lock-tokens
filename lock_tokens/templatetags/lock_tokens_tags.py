...
def lock_tokens_api_client():
    ...
    base_api_url = reverse(
        "lock-tokens:list-view", args=[randomstring, randomstring, 1]
    ).replace("%(randstring)s/%(randstring)s/1/" % {"randstring": randomstring}, "")
    ...
    if API_CSRF_EXEMPT:
        ...
        if hasattr(settings, "CSRF_HEADER_NAME"):
            # If django >= 1.9
            csrf_header_name = settings.CSRF_HEADER_NAME.replace("HTTP_", "").replace(
                "_", "-"
            )
